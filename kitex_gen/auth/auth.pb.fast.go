...
func (x *PubkeyResp) FastRead(buf []byte, _type int8, number int32) (offset int, err error) {
	switch number {
	case 1:
		offset, err = x.fastReadField1(buf, _type)
		if err != nil {
...
SkipFieldError:
	return offset, fmt.Errorf("%T cannot parse invalid wire-format data, error: %s", x, err)
ReadFieldError:
	return offset, fmt.Errorf("%T read field %d '%s' error: %s", x, number, fieldIDToName_PubkeyResp[number], err)
}

func (x *PubkeyResp) fastReadField1(buf []byte, _type int8) (offset int, err error) {
	x.PubKey, offset, err = fastpb.ReadString(buf, _type)
	return offset, err
}
...
func (x *PubkeyResp) FastWrite(buf []byte) (offset int) {
	if x == nil {
		return offset
	}
	offset += x.fastWriteField1(buf[offset:])
...
func (x *PubkeyResp) fastWriteField1(buf []byte) (offset int) {
	if x.PubKey == "" {
		return offset
	}
	offset += fastpb.WriteString(buf[offset:], 1, x.GetPubKey())
	return offset
}
...
func (x *PubkeyResp) sizeField1() (n int) {
	if x.PubKey == "" {
		return n
	}
	n += fastpb.SizeString(1, x.GetPubKey())
	return n
}
...
var fieldIDToName_PubkeyResp = map[int32]string{
	1: "PubKey",
}
